...
import { useCreateClient, useUpdateClient, useGetClient } from "@workspace/api-client-react";
...
function Input({ value, onChangeText, placeholder, keyboardType, multiline }: {
  value: string; onChangeText: (v: string) => void; placeholder?: string; keyboardType?: any; multiline?: boolean;
}) {
  return (
    <TextInput
      style={[styles.input, multiline && { height: 80, textAlignVertical: "top" }]}
      value={value} onChangeText={onChangeText} placeholder={placeholder}
      placeholderTextColor={Colors.textSecondary} keyboardType={keyboardType} multiline={multiline}
    />
  );
}
...
export default function CreateClientScreen() {
  ...
  const { data: existingClient } = useGetClient(editId ? parseInt(editId) : 0, { query: { enabled: isEditing } });
  ...
  const handleSave = async () => {
    if (!name.trim() || !phone.trim()) {
      Alert.alert("Error", "El nombre y teléfono son obligatorios"); return;
    }
    setSaving(true);
    try {
      const payload = { name: name.trim(), phone: phone.trim(), email: email.trim() || null, address: address.trim() || null, notes: notes.trim() || null };
      if (isEditing) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      ...
      >
        <Field label="Nombre *"><Input value={name} onChangeText={setName} placeholder="María García" /></Field>
        <Field label="Teléfono *"><Input value={phone} onChangeText={setPhone} placeholder="+52 55 1234 5678" keyboardType="phone-pad" /></Field>
        <Field label="Email"><Input value={email} onChangeText={setEmail} placeholder="maria@email.com" keyboardType="email-address" /></Field>
        <Field label="Dirección"><Input value={address} onChangeText={setAddress} placeholder="Calle, Colonia, Ciudad" /></Field>
        <Field label="Notas"><Input value={notes} onChangeText={setNotes} placeholder="Información adicional..." multiline /></Field>

        <Pressable style={[styles.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color={Colors.dark} /> : (
            <><Feather name="check" size={18} color={Colors.dark} /><Text style={styles.saveBtnText}>{isEditing ? "Guardar Cambios" : "Agregar Cliente"}</Text></>
          )}
        </Pressable>
      </ScrollView>
