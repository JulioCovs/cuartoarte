...
export default function CreatePaymentScreen() {
  const { eventId: prefillEventId, eventTitle: prefillTitle } = useLocalSearchParams<{
    eventId?: string;
    eventTitle?: string;
  }>();
  ...
  const [selectedEventId, setSelectedEventId] = useState<number | null>(
    prefillEventId ? parseInt(prefillEventId) : null
  );
  ...
  const handleSave = async () => {
    if (!selectedEventId || !amount || !date) {
      Alert.alert("Error", "Selecciona un evento, ingresa el monto y la fecha");
      return;
    }
          ...
          >
            {activeEvents.map((e) => (
              <Pressable
                ...
                style={[
                  styles.eventOption,
                  selectedEventId === e.id && styles.eventOptionActive,
                ]}
              ...
              >
                <Feather
                  name="calendar"
                  size={14}
                  color={selectedEventId === e.id ? Colors.dark : Colors.textSecondary}
                />
                <View style={{ flex: 1 }}>
                  <Text
                    style={[
                      styles.eventOptionTitle,
                      selectedEventId === e.id && { color: Colors.dark },
                    ]}
                    numberOfLines={1}
                  >
                    {e.title}
                  ...
                  <Text
                    style={[
                      ...
                      selectedEventId === e.id && { color: Colors.dark + "99" },
                    ]}
                  >
                    {e.date}
                  </Text>
          ...
          {PAYMENT_METHODS.map((m) => (
            ...
            >
              <Feather
                ...
                size={18}
                color={method === m ? Colors.dark : Colors.textSecondary}
              />
